#ifndef RTFSS_DEF_ASSIGN_MOVER
#define RTFSS_DEF_ASSIGN_MOVER
...
//Resolves self dependencies
class assign_mover : public ast_visitor{

	//FIXME FIXME Ignores indexes for now
	//assign_var redundant

	public:

		assign_mover(std::shared_ptr<ast> tree,
			...
			ast_visitor(tree,false,false),
				...
				vcscope(vcscope),
				pulse_id(pulse_id),
				assign(vcscope->get().size()),
				detached(vcscope->get().size(),false){}
	...
	private:
		...
		ast_property<unsigned int> assign_var;

		//For each var, the pointer to its most recent assignment
		std::vector<std::shared_ptr<ast_arith>> assign; 
		std::vector<bool> detached; 

		void fix_properties(std::shared_ptr<ast> src,std::shared_ptr<ast> dst);
		void rm_properties(std::shared_ptr<ast> target);
